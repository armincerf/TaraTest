...
export default function SupportQuerySpeedTest() {
	...
	const { completion, complete, isLoading, setCompletion } = useCompletion({
		...
		onFinish: (prompt, completion) => {
			if (currentAction === "generate_query") {
				setMessages((prevMessages) => [
					...prevMessages,
					{ type: "ai", content: completion },
				]);
				setCompletion("");
			} else if (currentAction === "score_response") {
				setScores((prevScores) => [...prevScores, completion]);
				setCompletion("");
			}
		},
	});
	...
	const generateQuery = () => {
		setCurrentAction("generate_query");
	...
	const scoreResponse = (query: string, response: string) => {
		setCurrentAction("score_response");
						...
						>
							<div className="max-h-[450px] flex-grow overflow-y-auto mb-2 bg-gray-100 p-2 rounded-lg text-sm">
								<AnimatePresence>
									...
									{isLoading && currentAction === "generate_query" && (
										<motion.div
											initial={{ opacity: 0, y: 10 }}
											animate={{ opacity: 1, y: 0 }}
											exit={{ opacity: 0, y: -10 }}
									...
									{isLoading && currentAction === "score_response" && (
										<motion.div
											initial={{ opacity: 0, y: 10 }}
											animate={{ opacity: 1, y: 0 }}
											exit={{ opacity: 0, y: -10 }}
									...
									{scores.length > queryCount && !isLoading && (
										<motion.div
											initial={{ opacity: 0, y: 10 }}
											animate={{ opacity: 1, y: 0 }}
											exit={{ opacity: 0, y: -10 }}
